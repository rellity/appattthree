    ...
    try {
      ...
      if (error.response) {
        ...
        Alert.alert(
          ...
          "Please check your internet/api connection"
        );
      } else {
  ...
  const generateBarcode = async () => {
    try {
      ...
      if (inputValue.length !== 9) {
        Alert.alert(
          "Error",
          "Input value (xxxxxxx-x) should have exactly 9 characters."
        );
        return;
      }
          ...
          [
            {
              ...
              onPress: async () => {
                const reg = await axios.get(
                  `${check}/attappthree/checkstustat.php`,
                  {
                    params: {
                      name: fullname,
                      value: inputValue,
  ...
  const handleShare = async () => {
    try {
      ...
      if (!folderInfo.exists) {
        await FileSystem.makeDirectoryAsync(barcodeFolder, {
          intermediates: true,
        });
      }
